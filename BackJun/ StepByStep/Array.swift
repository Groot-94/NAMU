...
func everegeUp() {
    ...
    for _ in 0..<n {
        let line = readLine() ?? ""
        print(String(format: "%.3f" ,everege(line.split(separator: " ").map{Int($0)!})),"%",separator: "")
    }
}
func everege(_ num :Array<Int>) -> Float {
    var result = 0
    for number in 1..<num.count {
        if num[number] > (num.reduce(0, + ) - num[0])/num[0] {
            result += 1
        }
    }
    return Float(result)/Float(num.count - 1) * 100.0
}
